...
@Component({
  selector: 'app-slice-management',
  templateUrl: './slice-management.component.html',
  styleUrls: ['./slice-management.component.scss'],
})
export class SliceManagementComponent implements OnInit {
...
// ↓ For Click View @2024/03/22 Add ↓
  ...
  /** @2024/05/03 Add
   *  導航到選定的切片詳細資訊頁面。
   *  @param schedule 從切片列表中選擇的切片物件。
   */
  viewScheduleDetailInfo( schedule: Schedule ) {

    this.selectSchedule = schedule; // 設定當前選擇的切片。
    ...
    this.router.navigate( ['/main/slice-mgr/info', this.selectSchedule.id, this.selectSchedule.tickettype] );
  }
