...
import { LogLists } from '../../log-management/log-management.component';
...
export class LogInfoComponent implements OnInit {
  ...
  Logroledetails(status: number): string {
  ...
  getSoftwareInfo() {
    if (this.commonService.isLocal) {
      /* local file test */
      this.logInfo = this.commonService.accountInfo; // add by yutchen @10/25 還未取得log數據先繼續套用accountInfo
    } else {
  ...
  back() {
    this.router.navigate(['/main/log-mgr']);
  }
