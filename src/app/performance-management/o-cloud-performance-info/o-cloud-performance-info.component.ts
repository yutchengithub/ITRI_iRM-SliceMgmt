...
export interface OcloudCpuUsage {
  ocloudId: string;
  startTime: string;
  endTime: string;
  interval: number;
...
export interface OcloudCpuLoading {
  ocloudId: string;
  startTime: string;
  endTime: string;
  interval: number;
...
export interface OcloudMemoryUsage {
  ocloudId: string;
  startTime: string;
  endTime: string;
  interval: number;
...
export interface OcloudDiskUsage {
  ocloudId: string;
  startTime: string;
  endTime: string;
  interval: number;
...
export interface OcloudDiskRate {
  ocloudId: string;
  startTime: string;
  endTime: string;
  interval: number;
...
export interface OcloudInterfaceUsage {
  ocloudId: string;
  startTime: string;
  endTime: string;
  interval: number;
...
export interface OcloudNetworkThroughput {
  ocloudId: string;
  startTime: string;
  endTime: string;
  interval: number;
...
@Component({
  selector: 'app-o-cloud-performance-info',
  templateUrl: './o-cloud-performance-info.component.html',
  styleUrls: ['./o-cloud-performance-info.component.scss']
})
export class OCloudPerformanceInfoComponent implements OnInit {
  ...
  overviewKpi: OverviewKpi = {} as OverviewKpi;
  tabs: Tabs[] = [
    {
      index: 0,
      diplayName: 'Overview',
    ...
    },
    {
      ...
      yLabel: 'CPU Usage (%)',
      yChartMax: 100,
      apiUrl: 'queryOcloudCpuUsage',
      loacalDataStr: 'ocloudCpuUsage'
    },
    {
      index: 2,
      ...
      yLabel: 'CPU Loading (%)',
      yChartMax: 100,
      apiUrl: 'queryOcloudCpuLoading',
      loacalDataStr: 'ocloudCpuLoading'
    },
    {
      index: 3,
      ...
      value: 'memoryUsage',
      yLabel: 'Memory (GB)',
      apiUrl: 'queryOcloudMemoryUsage',
      loacalDataStr: 'ocloudMemoryUsage'
    },
    {
      index: 4,
      ...
      apiUrl: 'queryOcloudDiskUsage',
      loacalDataStr: 'ocloudDiskUsage'
    },
    {
      index: 5,
      ...
      apiUrl: 'queryOcloudDiskRate',
      loacalDataStr: 'ocloudDiskRate'
    },
    {
      index: 6,
      ...
      yLabel: 'Interface (%)',
      yChartMax: 100,
      apiUrl: 'queryOcloudInterfaceUsage',
      loacalDataStr: 'ocloudInterfaceUsage'
    },
    {
      index: 7,
      ...
      apiUrl: 'queryOcloudNetworkThroughput',
      loacalDataStr: 'ocloudNetworkThroughput'
    },
    {
      index: 8,
      diplayName: 'Power (W)',
      value: 'power',
      yLabel: 'Power (W)',
      apiUrl: 'queryOcloudPower',
      loacalDataStr: 'ocloudPower'
    }
  ];
  indexMapTab: Map<number, Tabs> = new Map();
  valueMapTab: Map<string, Tabs> = new Map();
  overviewChart: any = { chartList: [] };
  ...
  getOverviewKpi() {
    console.log('Do getOverviewKpi.');
    if (this.commonService.isLocal) {
      /* local file test */
      window.setTimeout(() => {
        this.overviewKpi = this.commonService.overviewKpi;
        this.dealOverviewKpi();
      });
    } else {
      this.commonService.queryOverviewKpi(this.cloudId).subscribe(
        res => {
          console.log('getOverviewKpi:');
          console.log(res);
          this.overviewKpi = res as OverviewKpi;
          this.dealOverviewKpi();
        }
      );
    }
  ...
  dealOverviewKpi() {
    this.overviewChart = { chartList: [] };
    const startTime = this.overviewKpi.startTime;
    const endTime = this.overviewKpi.endTime;
    const interval = this.overviewKpi.interval;
    const tickInterval = this.overviewKpi.tickInterval;
    const labels = this.getChartLabels(startTime, endTime, interval, tickInterval);
    this.tabs.forEach((row) => {
      const value = row.value;
      const data: number[] = [];
      if (value !== 'all') {
        this.overviewKpi[value].forEach((v) => {
          const rep = this.dataReplaceStr(v);
          data.push(Number(v.replace(rep, '')));
        });
        // small chart
        this.overviewChart.chartList.push(value);
        this.overviewChart[value] = {
          data: {
            labels: labels,
            datasets: [
              {
    ...
    // console.log(this.tabs);
    // refresh
    this.refreshTimeout = window.setTimeout(() => this.getOverviewKpi(), this.refreshTime * 1000);
  }
  ...
  query(value: string) {
    ...
    if (this.commonService.isLocal) {
      ...
      let url = `${this.commonService.restPath}/${tab.apiUrl}/${this.sessionId}/${this.cloudId}&type=${tab.intervalType}`;
      if (tab.intervalType === 'custom') {
        url += timeStr;
      }
  ...
  back() {
    this.router.navigate(['/main/performance-mgr', 'ocloud', 'All', 'All']);
  }
  ...
  drawColorChange() {
    ...
    if (this.activeIndex === 0) {
      const chartList: string[] = this.overviewChart.chartList;
      chartList.forEach((c) => {
        this.overviewChart[c].options.scales.x.ticks.color = linechartcolor;
        this.overviewChart[c].options.scales.x.grid.borderColor = linechartcolor;
        this.overviewChart[c].options.scales.y.ticks.color = linechartcolor;
        this.overviewChart[c].options.scales.y.grid.borderColor = linechartcolor;
      });
    } else {
